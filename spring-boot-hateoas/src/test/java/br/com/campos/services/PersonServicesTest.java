...
class PersonServicesTest {
    ...
    void createV2() {
        ...
        PersonDTOV2 persistedDTOV2 = input.mockDTOV2(1);
        persistedDTOV2.setId(1L);

        when(personMapper.convertDTOToEntity(dtoV2)).thenReturn(person);
        when(repository.save(person)).thenReturn(persisted);
        when(personMapper.convertEntityToDTO(persisted)).thenReturn(persistedDTOV2);
        
        var result = services.createV2(dtoV2);
        ...
        assertTrue(result.getLinks().stream()
                .anyMatch(link -> link.getRel().value().equals("self") &&
                        link.getHref().endsWith("/api/person/v1/v2") &&
                        link.getType().equals("POST")
                ));
    ...
    void findAll() {
        ...
        assertNotNull(personNine.getLinks().stream()
                .anyMatch(link -> link.getRel().value().equals("self") &&
                        link.getHref().endsWith("/api/person/v1/9") &&
                        link.getType().equals("GET")
                ));
